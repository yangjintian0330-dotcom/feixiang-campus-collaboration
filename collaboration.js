...
dlg.innerHTML=`<div class="co-head"><div><h2 id="co-title">协作</h2></div><button class="co-close" aria-label="关闭协作设置">×</button></div><div class="co-body"><div class="co-invite-panel"><span class="co-label" id="co-picker-label">邀请同事协作</span><div class="co-picker" id="co-picker"><div class="co-picker-field" id="co-picker-field" role="combobox" aria-labelledby="co-picker-label" aria-expanded="false" aria-controls="co-picker-popover" aria-haspopup="listbox"><div id="co-selected" class="co-selected"></div><input id="co-search" placeholder="搜索姓名" aria-label="搜索老师" autocomplete="off" aria-controls="co-results"><svg class="co-picker-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></div><div id="co-picker-popover" class="co-picker-popover" hidden><div class="co-quick" role="group" aria-label="批量选择老师"><span>批量选择</span><button type="button" id="co-all-school">学校内全部成员</button><button type="button" id="co-all-subject">同学科全部成员</button></div><div id="co-results" class="co-results" role="listbox" aria-multiselectable="true"></div><div class="co-picker-footer"><span id="co-selection-count" aria-live="polite">已选择 0 位老师</span><button type="button" id="co-clear">清空已选</button></div></div></div><div class="co-invite-actions"><div class="co-permission"><select id="co-invite-role" class="co-select" aria-label="邀请成员权限" aria-describedby="co-role-hint"><option value="comment">可评论</option><option value="edit">可编辑</option></select><span id="co-role-hint" class="co-sr-only">可发表评论，不可修改原件</span></div><button id="co-add" class="co-primary" disabled>添加协作者</button></div></div><section class="co-section"><div class="co-section-title">有权访问 <span id="co-count" class="co-count"></span></div><div class="co-member-list"><div class="co-member"><span class="co-avatar">杨</span><div class="co-person"><strong>杨金田 <span class="co-sub">（我）</span></strong></div><span class="co-owner">所有者</span></div><div id="co-members"></div></div><div id="co-members-empty" class="co-hint" hidden>尚未添加协作者，可在上方选择老师</div></section><div id="co-feedback" class="co-feedback" role="status" aria-live="polite"></div><input id="co-linkbox" class="co-linkbox" aria-label="示例链接，请手动复制" readonly hidden></div><div class="co-foot"><button id="co-copy-link" class="co-copy">↗ &nbsp;复制链接</button></div>`;
...
function renderSelection(){
 ...
 $('co-clear').disabled=!selected.size;updateGroups();
...
function search(){
 ...
 choices.forEach(person=>{const b=document.createElement('button');b.type='button';b.className='co-result';const joined=state.members.some(m=>m.id===person.id);b.disabled=joined;b.setAttribute('aria-pressed',String(selected.has(person.id)));b.dataset.person=person.id;b.innerHTML=`<span class="co-check" aria-hidden="true">${selected.has(person.id)?'✓':''}</span><span>${person.name}</span>`;if(joined){const tag=document.createElement('span');tag.className='co-joined';tag.textContent='已加入';b.append(tag);}b.onclick=e=>{e.stopPropagation();if(selected.has(person.id))selected.delete(person.id);else selected.add(person.id);renderSelection();search();const next=[...$('co-results').querySelectorAll('button')].find(el=>el.textContent.includes(person.name));next?.focus();};$('co-results').append(b);});
}
...
function setPicker(open,focus=true){$('co-picker-popover').hidden=!open;$('co-picker-field').setAttribute('aria-expanded',String(open));if(open){updateGroups();search();if(focus)$('co-search').focus();}}
$('co-picker-field').onclick=e=>{if(!e.target.closest('.co-chip button'))setPicker(true);};
$('co-search').addEventListener('focus',()=>setPicker(true,false));
dlg.addEventListener('cancel',e=>{if(!$('co-picker-popover').hidden){e.preventDefault();$('co-search').focus({preventScroll:true});setPicker(false,false);}});
