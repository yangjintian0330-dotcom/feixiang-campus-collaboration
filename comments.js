...
const icons={edit:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="m14 5 5 5M4 20l5-1L20 8a3.5 3.5 0 0 0-5-5L4 14v6Z"/><path d="M13 21H5a3 3 0 0 1-3-3v-7"/></svg>',comment:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M20 11a8 8 0 0 1-8 8H5l-3 3V11a9 9 0 0 1 18 0Z"/><path d="M7 9h8M7 13h5"/></svg>'};
$('add-comment').innerHTML=icons.comment+'<span>添加批注</span>';$('edit-mode').innerHTML="<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" width=\"16\" height=\"16\" viewBox=\"0 0 16 16\"><defs><clippath id=\"clipPath1502922650\"><path d=\"M0 0L16 0L16 16L0 16L0 0Z\" fill-rule=\"nonzero\" transform=\"matrix(1 0 0 1 0 0)\"></path></clippath></defs><g clip-path=\"url(#clipPath1502922650)\"><path d=\"M2 0.7Q1.46152 0.7 1.08076 1.08076Q0.7 1.46152 0.7 2L0.7 10.1963Q0.7 10.4246 0.8518 10.5952L2.41267 12.349Q2.59165 12.5501 2.86087 12.5501Q3.13009 12.5501 3.30907 12.349L4.86994 10.5952Q5.02174 10.4246 5.02174 10.1963L5.02174 2Q5.02174 1.46152 4.64098 1.08076Q4.26022 0.7 3.72174 0.7L2 0.7ZM2 -0.7L3.72174 -0.7Q4.27059 -0.7 4.77292 -0.487533Q5.25758 -0.282538 5.63093 0.0908119Q6.00428 0.464163 6.20927 0.948824Q6.42174 1.45115 6.42174 2L6.42174 10.1963Q6.42174 10.9574 5.91574 11.5259L4.35487 13.2797Q4.06301 13.6076 3.66375 13.7824Q3.28067 13.9501 2.86087 13.9501Q2.44107 13.9501 2.05799 13.7824Q1.65873 13.6076 1.36687 13.2797L-0.194 11.5259Q-0.7 10.9574 -0.7 10.1963L-0.7 2Q-0.7 1.45115 -0.487533 0.948824Q-0.282539 0.464163 0.0908117 0.0908119Q0.464162 -0.282539 0.948823 -0.487533Q1.45115 -0.7 2 -0.7Z\" fill-rule=\"nonzero\" transform=\"matrix(0.866025 0.5 -0.5 0.866025 7.59814 0.798828)\" fill=\"rgb(20, 31, 27)\"></path><path d=\"M-0.7 0C-0.7 0.386596 -0.386596 0.7 0 0.7L5.68682 0.700001C6.07342 0.700001 6.38682 0.386597 6.38682 0C6.38682 -0.386595 6.07342 -0.699999 5.68682 -0.699999L0 -0.7C-0.386596 -0.7 -0.7 -0.386596 -0.7 0Z\" fill-rule=\"evenodd\" transform=\"matrix(-0.868192 -0.496229 0.496229 -0.868192 10.8735 6.62402)\" fill=\"rgb(20, 31, 27)\"></path><path d=\"M-0.7 0C-0.7 0.386596 -0.386596 0.7 0 0.7L4.21455 0.700001C4.60115 0.700001 4.91455 0.386597 4.91455 0C4.91455 -0.386595 4.60115 -0.699999 4.21455 -0.699999L0 -0.7C-0.386596 -0.7 -0.7 -0.386596 -0.7 0Z\" fill-rule=\"evenodd\" transform=\"matrix(-1 0 -0 -1 13.3462 13.748)\" fill=\"rgb(20, 31, 27)\"></path><path d=\"M-0.7 0C-0.7 0.386596 -0.386596 0.7 0 0.7L2.59886 0.700001C2.98546 0.700001 3.29886 0.386597 3.29886 0C3.29886 -0.386595 2.98546 -0.699999 2.59886 -0.699999L0 -0.7C-0.386596 -0.7 -0.7 -0.386596 -0.7 0Z\" fill-rule=\"evenodd\" transform=\"matrix(-1 0 0 -1 12.5386 11.5645)\" fill=\"rgb(20, 31, 27)\"></path></g></svg>";document.querySelector('.bubble-icon').innerHTML=icons.comment;
...
function setMode(value){if(!discard())return false;cancel();mode=Boolean(value)&&canComment();editMode=false;document.body.classList.toggle('commenting',mode);$('mode-tip').hidden=true;const btn=$('add-comment');btn.setAttribute('aria-pressed',String(mode));btn.setAttribute('aria-label','添加批注');btn.title=mode?'退出批注模式':'添加批注';btn.querySelector('span').textContent='添加批注';if(frameDoc)frameDoc.body.style.cursor=mode?'crosshair':'';if(window.parent!==window)window.parent.postMessage({type:'fx-annotation-state',active:mode},location.origin);if(mode)toast('点击或拖动框选内容，添加批注；Esc 退出');return true;}
...
function renderComment(){const t=threads.find(t=>t.id===floatingId);const card=$('comment-card');card.replaceChildren();if(!t||!canComment()){card.hidden=true;return;}
 ...
 if(t.replies.length){const list=el('div','saved-replies');t.replies.forEach(r=>list.append(messageBlock(r,t,true)));card.append(list);}
 const form=el('form','saved-reply'),input=el('textarea'),send=el('button','','✓');input.placeholder='回复…';input.setAttribute('aria-label','回复此评论');input.maxLength=2000;send.type='submit';send.disabled=true;send.setAttribute('aria-label','确定回复');input.oninput=()=>send.disabled=!input.value.trim();form.append(input,send);form.onsubmit=e=>{e.preventDefault();if(!canComment()||!input.value.trim())return;t.replies.push({id:crypto.randomUUID(),author:user,text:input.value.trim(),time:Date.now()});if(!save()){t.replies.pop();return;}render();renderComment();positionComment();};card.append(form);
...
function messageBlock(message,parent,isReply=false){
 ...
 person.append(el('span','avatar',people[message.author].name[0]),el('strong','',people[message.author].name+(message.author===user?'（我）':'')),el('small','',new Date(message.time).toLocaleDateString('zh-CN',{month:'numeric',day:'numeric'})+(message.editedAt?' · 已编辑':'')));
 ...
 if(message.author===user&&canComment()){
  ...
  actions.append(action('编辑',()=>{
   ...
   if(!canComment()||message.author!==user)return;
   menu.open=false;if(!confirm(isReply?'删除这条回复？':'删除这条评论及其回复？'))return;
...
function render(){const visible=threads;$('count').textContent=threads.filter(t=>!t.resolved).length;$('panel-count').textContent=threads.length;$('thread-list').replaceChildren();document.querySelectorAll('[data-filter]').forEach(b=>b.classList.toggle('active',b.dataset.filter===filter));if(!visible.length){const empty=el('div','empty');empty.append(el('strong','',filter==='open'?'一起打磨这份课件':'暂无已解决评论'),el('span','',filter==='open'?'点击顶部「添加批注」，再选择课件中的具体内容。':'作者解决的讨论会保留在这里。'));$('thread-list').append(empty);}
visible.forEach(t=>{const card=el('article','thread'+(active===t.id?' selected':''));card.dataset.thread=t.id;const quote=action(t.anchor.quote,()=>focusThread(t));quote.className='thread-quote';quote.title=t.anchor.quote;const body=messageBlock(t,t);card.append(quote,body);const menu=body.querySelector('.comment-more');if(menu){menu.classList.add('thread-more');card.append(menu);}const actions=el('div','thread-actions');actions.style.marginTop='10px';actions.append(action('回复',()=>{if(!canComment())return;const old=card.querySelector('.reply-form');if(old){old.querySelector('textarea').focus();return;}const form=el('form','reply-form'),input=el('textarea');input.placeholder=`回复 ${people[t.author].name}`;input.setAttribute('aria-label','回复内容');input.maxLength=2000;const send=el('button','primary','发送回复');send.type='submit';send.disabled=true;input.oninput=()=>send.disabled=!input.value.trim();form.append(input,send);form.onsubmit=e=>{e.preventDefault();if(!canComment()||!input.value.trim())return;const reply={id:crypto.randomUUID(),author:user,text:input.value.trim(),time:Date.now()};t.replies.push(reply);if(save()){render();toast('回复已发送');}else t.replies.pop();};card.append(form);input.focus();}));if(canEdit())actions.append(action(t.resolved?'重新打开':'标为已解决',()=>{if(!canEdit())return;t.resolved=!t.resolved;if(!save())t.resolved=!t.resolved;render();}));card.append(actions);if(canEdit()){const revise=action('按照评论修改',()=>openEditChat({id:t.id,anchor:structuredClone(t.anchor),text:t.text,author:t.author}));revise.className='revise-comment';card.append(revise);}if(t.replies.length){const replies=el('div','replies');t.replies.forEach(r=>{if(people[r.author])replies.append(messageBlock(r,t,true));});card.append(replies);}$('thread-list').append(card);});renderMarkers();if(floatingId){renderComment();positionComment();}}
